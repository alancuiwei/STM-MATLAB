function outputdata=MA_MACD(inputdata)
...
%==========================================================================
Price2=zeros(size(inputdata.commodity.serialmkdata.cp));
...
%                 Price(2,TradeDay(i-1)-1)<Price(1,TradeDay(i-1)-1))%向上突破的条件判断
%             TradeDay(Cnt)=PositionTrade(i);
%             Cnt=Cnt+1;
%         else if (Price(1,PositionTrade(i))>Price(1,PositionTrade(i)+1)&&...
%                    Price(2,PositionTrade(i)+1)<Price(2,PositionTrade(i)+1)&&...
%                     Price(2,TradeDay(i-1)-1)>Price(1,TradeDay(i-1)-1))%向下突破的条件判断
%                 TradeDay(Cnt)=PositionTrade(i);
%                 Cnt=Cnt+1;
%             else
%                 TradeDay(Cnt)=TradeDay(i-1);
...
%==========================================================================
%由方向策略和执行策略确定真正交易日期         %方法一
% RealTrade_a=[];
% RealTrade_b=[];
% Cnt=1;
% for i = 1:numel(Position_a)
%     PA_ID=find(TradeDay<Position_a(i));
%     if ~isempty(PA_ID) && ismember(TradeDay(PA_ID(end)),TradeDay_a)
%         RealTrade_a(Cnt)=Position_a(i);
%         Cnt=Cnt+1;
%     end
% end
% 
% Cnt=1;
% for i = 1:numel(Position_b)
%     PB_ID=find(TradeDay<Position_b(i));
%     if ~isempty(PB_ID) && ismember(TradeDay(PB_ID(end)),TradeDay_b)
%         RealTrade_b(Cnt)=Position_b(i);
%         Cnt=Cnt+1;
%     end
% end
...
%     end
% end
% TickOutIdx=find(Flag==-1);
% Trade(TickOutIdx)=[];

%==========================================================================
%由方向策略和执行策略确定真正交易日期         %方法二
RealTrade_a=[];
RealTrade_b=[];
Cnt=1;
...
% cnt=1;
% for i=1:numel(Position1)
%     if (Price1(Position1(i)+1)>Price2(Position1(i)+1))
%        Position_a(cnt)=Position1(i);
%        cnt=cnt+1;
%     else if (Price1(Position1(i)+1)<Price2(Position1(i)+1))
%               Position_b(cnt)=Position1(i);
%        cnt=cnt+1;
%         end
%     end
