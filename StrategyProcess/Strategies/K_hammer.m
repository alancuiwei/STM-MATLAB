function outputdata=K_hammer(inputdata)
tmp=load('KDJDy.mat');
...
%计算20日最低值
for i=1:(numel(Price(4,:))-5)
    L(i)=min(Price(4,i:i+4));
end
Price4=Price(4,6:length(Price(4,:)));
cnt=1;
for i=1:numel(diff1)
    if (diff3(i)>=2*diff1(i)&&diff2(i)<=2*tick&&Price4(i)<=L(i))
        Position(cnt)=i+5;
        cnt=cnt+1;
    end
end
