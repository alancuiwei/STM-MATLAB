function out_traderecord=ZR_STRATEGY_010606(in_inputdata)
% 羽根英树的策略:010605 类型3
...
if ~isempty(l_opdays)
    for l_dayid=1:length(l_opdays)
        ...
        if (g_tradecaculation(1).opdays(l_opdays(l_dayid))==3)
            g_tradecaculation.pos.type(l_posid)=41; 
        else
            g_tradecaculation.pos.type(l_posid)=42; 
        end
        % 建仓日期
...
% 调用参数
l_counters=in_inputdata.strategyparms.counters;
...
for l_gapid=1:length(l_days)
    ...
    if (l_num==1)
        % 首次建仓,0点附近
        if (l_days(l_gapid)>1)&&(min(in_inputdata.pair.mkdata.vl(l_days(l_gapid),:))>=100)
...
%                     l_lastdowncpgap=l_cpgap(l_days(l_gapid));                                            
                ...
                l_downcounter=0;
                l_lastdowncpgap=l_cpgap(l_days(l_gapid));                 
                if (l_upcounter>=l_counters)&&(min(in_inputdata.pair.mkdata.vl(l_days(l_gapid),:))>=100) 
        ...
        elseif (l_cpgap(l_days(l_gapid))<l_lastdowncpgap)
            ...
            if (l_isop)
                l_opdaygap=l_cpgap(find(l_opdays,1,'last'));
                if (l_cpgap(l_days(l_gapid))-l_opdaygap)<(-l_wins*g_indicatordata.pair.ATR(1))
...
%                 l_upcounter=0;
%                 l_lastupcpgap=l_cpgap(l_days(l_gapid));                 
                if (l_downcounter>=l_counters)&&(min(in_inputdata.pair.mkdata.vl(l_days(l_gapid),:))>=100)
        ...
        if (min(in_inputdata.pair.mkdata.vl(l_days(l_gapid),:))>=100)
            l_opdays(l_days(l_gapid))=1;
            l_isop=1;
        end
        l_upcounter=0;
