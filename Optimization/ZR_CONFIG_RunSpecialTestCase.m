function ZR_CONFIG_RunSpecialTestCase()
% 记录运行时参数
...
% 根据策略名，生成策略信息
l_strategyinfo=ZR_FUN_QueryArbitrageInfo(g_strategyid);
...
switch g_strategyid(1:2)      %套利类型
    case '01'           %跨期套利
        g_method.runstrategy.fun=@ZR_STRATEGY_PAIR;
        coredata.type='pair';
    case '04'           %单边策略
        g_method.runstrategy.fun=@ZR_STRATEGY_SERIAL;  
        coredata.type='serial';
end
g_method.rundataprocess=@ZR_DATAPROCESS;        %对于数据的处理，不区分策略
