...
Price(Price==0)=inf;
% xlswrite('D:\zx\ZR_LIB_0619\040704\TestResults_SERIAL',outSlowD,'Sheet1','F');
% xlswrite('D:\zx\ZR_LIB_0619\040704\TestResults_SERIAL',outSlowK,'Sheet1','G');
...
if(SignPrice(PositionTrade(1))~=0) %判断此交点位置是否刚好为整数
        if(Price(1,PositionTrade(i)+1)>Price(1,PositionTrade(i)) && Price(2,PositionTrade(i)+1)>Price(2,PositionTrade(i))&&...
                ...
                TradeDay1(i)=PositionTrade(i);
                    ...
                    Price(2,PositionTrade(i)+1)>Price(1,PositionTrade(i)+1))%向下突破的条件判断
                    TradeDay1(i)=PositionTrade(i);
            else
                TradeDay1(i)=0;
            end
...
for i=2:numel(PositionTrade)
    if(SignPrice(PositionTrade(i))~=0) %判断此交点位置是否刚好为整数
        if(Price(1,PositionTrade(i)+1)>Price(1,PositionTrade(i)) && Price(2,PositionTrade(i)+1)>Price(2,PositionTrade(i))&&...
                ...
                Price(1,TradeDay(i-1))>Price(1,TradeDay(i-1)+1)&&Price(2,TradeDay(i-1))>Price(2,TradeDay(i-1)+1)&&Price(2,TradeDay(i-1)+1)>Price(1,TradeDay(i-1)+1))%向上突破的条件判断
                TradeDay(Cnt)=PositionTrade(i);
                Cnt=Cnt+1;
        else if(Price(1,PositionTrade(i))>Price(1,PositionTrade(i)+1)&&Price(2,PositionTrade(i))>Price(2,PositionTrade(i)+1)&&...
                    Price(1,PositionTrade(i)+1)<Price(2,PositionTrade(i)+1)&&...
                    Price(1,TradeDay(i-1)+1)>Price(1,TradeDay(i-1)) && Price(2,TradeDay(i-1)+1)>Price(2,TradeDay(i-1))&&Price(1,TradeDay(i-1)+1)>Price(2,TradeDay(i-1)+1))%向下突破的条件判断
                    TradeDay(Cnt)=PositionTrade(i);
                    Cnt=Cnt+1;
            else
