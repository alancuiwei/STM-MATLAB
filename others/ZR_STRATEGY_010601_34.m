function ZR_STRATEGY_010601_34(varargin)
% 羽根英树的策略:010601
        ...
        elseif (l_cpgap(l_days(l_gapid))<l_lastdowncpgap)
            ...
            if (l_isop)
                ...
                if (l_cpgap(l_days(l_gapid))-l_opdaygap)<(-l_wins*g_indicatordata.pair(in_pairid).ATR(1))
                    ...
                    l_lastdowncpgap=l_cpgap(l_days(l_gapid)); 
                    l_isop=0;                    
                elseif (l_downcounter>=l_counters) 
                    % 达到止盈平仓的条件，downcouter重新计数
                    l_cpdays(l_days(l_gapid))=4;
                    l_isop=0;
                    l_upcounter=0;
            ...
            elseif (l_downcounter>=2*l_counters)&&(min(g_rawdata.pair(in_pairid).mkdata.vl(l_days(l_gapid),:))>=100)
...
% 最后一个交易日，不可以建仓
if l_isop
    if (l_opdays(end)>0)||(l_opdays(end-1)>0)
        l_opdays(end-1:end)=0;
