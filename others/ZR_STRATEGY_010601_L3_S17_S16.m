function ZR_STRATEGY_010601_L3_S17_S16(varargin)
% 羽根英树的策略:010601
...
% 调用参数
l_countersofdown=g_commodityparams.countersofdown;
l_countersofup=g_commodityparams.countersofup;
l_losses=g_commodityparams.losses;
l_sharp=g_commodityparams.sharp;
l_sharpdays=g_commodityparams.sharpdays;
    ...
    elseif(l_days(l_gapid)<=(l_lastid+1))
        if l_cpgap(l_days(l_gapid))>l_lastupcpgap
            ...
            if l_isop              
                ...
                if (l_upcounter>=l_countersofup)&&(min(g_rawdata.pair(in_pairid).mkdata.vl(l_days(l_gapid),:))>=100) 
        ...
        elseif (l_cpgap(l_days(l_gapid))<l_lastdowncpgap)               
            ...
            if ((l_lastdowncpgap-l_cpgap(l_days(l_gapid)))>l_sharp)&&(~l_isop)
                ...
                if (l_isop)
                    ...
                    if (~l_loss)&&(l_downcounter>=l_countersofdown)&&(min(g_rawdata.pair(in_pairid).mkdata.vl(l_days(l_gapid),:))>=100)
                        % 达到建仓的条件，upcouter重新计数
