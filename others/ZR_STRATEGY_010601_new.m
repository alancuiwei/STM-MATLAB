function ZR_STRATEGY_010601_new(varargin)
% 羽根英树的策略:010601
...
% 止损tick
g_indicatordata.pair(in_pairid).ATR=max(g_rawdata.commodity.info.tick)*ones(g_rawdata.pair(in_pairid).datalen,1);
...
if ~isempty(l_opdays)
    for l_dayid=1:length(l_opdays)
        ...
        % 保证金
        g_tradedata(in_pairid).pos.margin(l_posid)=round(l_tradeunit(1)*l_margin(1)*g_rawdata.pair(in_pairid).mkdata.cp(l_cpdays(l_dayid),1)...
            +l_tradeunit(2)*l_margin(2)*g_rawdata.pair(in_pairid).mkdata.cp(l_cpdays(l_dayid),2));
        % 平仓时的近期合约交易量
        ...
        for l_chargeid=1:length(l_tradecharge)
            if (l_tradecharge(l_chargeid)<1)
                g_tradedata(in_pairid).pos.optradecharge(l_posid)=round(l_tradecharge(l_chargeid)*...
                    g_rawdata.pair(in_pairid).mkdata.op(l_opdays(l_dayid),1))+g_tradedata(in_pairid).pos.optradecharge(l_posid);
                g_tradedata(in_pairid).pos.cptradecharge(l_posid)=round(l_tradecharge(l_chargeid)*...
                    g_rawdata.pair(in_pairid).mkdata.cp(l_cpdays(l_dayid),2))+g_tradedata(in_pairid).pos.optradecharge(l_posid);                
            else
                g_tradedata(in_pairid).pos.optradecharge(l_posid)=l_tradecharge(l_chargeid)+g_tradedata(in_pairid).pos.optradecharge(l_posid);
                g_tradedata(in_pairid).pos.cptradecharge(l_posid)=l_tradecharge(l_chargeid)+g_tradedata(in_pairid).pos.optradecharge(l_posid);
            end
        end
        ...
        g_tradedata(in_pairid).pos.profit(l_posid)=...
            (-(g_rawdata.pair(in_pairid).mkdata.cp(l_cpdays(l_dayid),2)-g_rawdata.pair(in_pairid).mkdata.cp(l_opdays(l_dayid),2))...
            *l_tradeunit(2)*g_rawdata.pair(in_pairid).ctunit2...
            +(g_rawdata.pair(in_pairid).mkdata.cp(l_cpdays(l_dayid),1)-g_rawdata.pair(in_pairid).mkdata.cp(l_opdays(l_dayid),1))...
            *l_tradeunit(1)*g_rawdata.pair(in_pairid).ctunit1)*l_sign...
