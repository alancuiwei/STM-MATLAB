function ZR_STRATEGY_010601_L1_Advance(varargin)
% 羽根英树的策略:010601
...
% 调用参数
g_strategydata(in_pairid).type=zeros(size(g_rawdata.pair(in_pairid).mkdata.cpgap));
for l_period=2:g_commodityparams.period
%     l_period=g_commodityparams.period;
    ...
    if (g_rawdata.pair(in_pairid).datalen>l_period)
        ...
        SUB_ComputeStrategyDataPerPair(in_pairid);        
    else
        % l_periodmin=l_period;
        % 计算gap为负时的，周期内的最大值
        l_cpgapdata=g_rawdata.pair(in_pairid).mkdata.cpgap;
...
g_strategydata(in_pairid).case1a.ismatched=g_strategydata(in_pairid).type>=(g_commodityparams.period-1);
...
g_strategydata(in_pairid).case1b.ismatched=(g_strategydata(in_pairid).isnegativemaxgappoint)&(g_rawdata.pair(in_pairid).mkdata.cpgap<0)...
    ...
    &g_indicatordata.pair(in_pairid).isallowed; 
g_strategydata(in_pairid).type=g_strategydata(in_pairid).type+1*(g_strategydata(in_pairid).case1a.ismatched|g_strategydata(in_pairid).case1b.ismatched)...
    &g_indicatordata.pair(in_pairid).serial;
% 11*g_strategydata(in_pairid).case1a.ismatched+12*g_strategydata(in_pairid).case1b.ismatched
