...
if exist(strcat(pwd,'/',G_ExtractHistoryDataFromMySQL.outfilename),'file')
    % 导入数据库数据
    load(strcat(pwd,'/',G_ExtractHistoryDataFromMySQL.outfilename));    
else
    g_database=[];
end
...
if G_ExtractHistoryDataFromMySQL.isupdated
    ...
    if exist(strcat(pwd,'/','g_coredata_specialtestcase.mat'),'file')
        delete(strcat(pwd,'/','g_coredata_specialtestcase.mat'));
    end
    save(strcat(pwd,'/',G_ExtractHistoryDataFromMySQL.outfilename),'g_database');
    disp('数据库文件更新：');
    disp(dir(strcat(pwd,'/',G_ExtractHistoryDataFromMySQL.outfilename)));     
else 
    disp('数据库文件没有更新：'); 
end
