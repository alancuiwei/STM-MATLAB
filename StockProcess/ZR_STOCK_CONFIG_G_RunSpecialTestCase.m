function ZR_STOCK_CONFIG_G_RunSpecialTestCase()
% 记录运行时参数
...
switch l_strategyid(1:2)      %套利类型
    case '01'       %跨期套利
        ...
        G_RunSpecialTestCase.g_method.runstrategy.fun=@ZR_STOCK_STRATEGY_PAIR;    
        G_RunSpecialTestCase.g_method.rundataprocess=@ZR_STOCK_DATAPROCESS;
    case '04'       %单边策略
        G_RunSpecialTestCase.coredata.type='serial'; 
        G_RunSpecialTestCase.g_method.runstrategy.fun=@ZR_STOCK_STRATEGY_SERIAL;
        G_RunSpecialTestCase.g_method.rundataprocess=@ZR_STOCK_DATAPROCESS;
    case '10'       %？？？？？
end
% l_cmdstr=strcat('G_RunSpecialTestCase.g_method.rundataprocess=@ZR_STOCK_DATAPROCESS_',l_strategyid,';');
% eval(l_cmdstr);

% G_RunSpecialTestCase.g_method.runstrategy.fun=@ZR_STOCK_STRATEGY_SERIAL;
% G_RunSpecialTestCase.g_method.runstrategy.fun=@ZR_STOCK_STRATEGY_PAIR;
% G_RunSpecialTestCase.g_method.rundataprocess=@ZR_STOCK_DATAPROCESS_010603;
% G_RunSpecialTestCase.g_method.rundataprocess=@ZR_STOCK_DATAPROCESS_040704;
%G_RunSpecialTestCase.coredata.type='pair';
...
if iscell(g_XMLfile)
    for l_xmlid = 1:numel(g_XMLfile)
        ...
        if ~isempty(l_titlenames)
            for l_titleid=1:length(l_titlenames)
                l_commandstr=strcat(l_commandstr,...
                    sprintf('l_var=eval(num2str(g_XMLfile{%d}.g_strategyparams.%s));G_RunSpecialTestCase.g_strategyparams{%d}.%s=l_var(1)*%s',...
                    l_xmlid,l_titlenames{l_titleid},...
                    l_xmlid,l_titlenames{l_titleid},'ones(length(G_RunSpecialTestCase.g_commoditynames),1);')); 
            end
        end
        eval(l_commandstr);
    ...
    if ~isempty(l_titlenames)
        for l_titleid=1:length(l_titlenames)
            l_commandstr=strcat(l_commandstr,...
                sprintf('G_RunSpecialTestCase.g_strategyparams.%s=g_XMLfile.g_strategyparams.%s(1)*%s',...
                l_titlenames{l_titleid},l_titlenames{l_titleid},'ones(length(G_RunSpecialTestCase.g_commoditynames),1);')); 
        end
    end
    eval(l_commandstr);  
...
G_RunSpecialTestCase.g_tradedata.pos.direction=[];
...
G_RunSpecialTestCase.g_report.commodity.record.pos.direction=[];
...
G_RunSpecialTestCase.g_report.record.pos.direction=[];
