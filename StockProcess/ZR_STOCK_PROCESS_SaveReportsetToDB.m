function ZR_STOCK_PROCESS_SaveReportsetToDB()
% 将交易记录存入数据库中
global g_tables;
...
l_colnames={'rightid';'tradeobject';'isclosepos';'openposdate';'closeposdate';'openposprice';'closeposprice';'marginaccount';'direction';'profit';'strategyid';'userid';'ordernum'};
l_strategyid=repmat({g_tables.strategyid},length(g_tables.tabledata.record.pos(2:end,1)),1);
l_userid=repmat({g_tables.userid},length(g_tables.tabledata.record.pos(2:end,1)),1);
l_ordernum=repmat({g_tables.ordernum},length(g_tables.tabledata.record.pos(2:end,1)),1);
l_exdata=cat(2,g_tables.tabledata.record.pos(2:end,[1,2,3,4,5,6,7,8,11,12]),l_strategyid,l_userid,l_ordernum);
