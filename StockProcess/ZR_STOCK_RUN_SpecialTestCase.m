function ZR_STOCK_RUN_SpecialTestCase(varargin)
% 只执行一个测试case
...
%     g_strategyparams=G_RunSpecialTestCase.g_strategyparams(l_methodid);
    g_strategyparams=G_RunSpecialTestCase.g_strategyparams;
    g_method.runstrategy(l_methodid).fun(varargin{:});
    ZR_STOCK_PROCESS_MergeReportset();
end

% 计算总体测评指标
ZR_STOCK_PROCESS_ComputeReferenceInwindow(G_RunSpecialTestCase.coredata.startdate,G_RunSpecialTestCase.coredata.enddate);
% 保存测试报告
ZR_STOCK_PROCESS_ShowReport();
% 提示结束
ZR_STOCK_FUN_Disp('测试结束!','Test case Finished!');
