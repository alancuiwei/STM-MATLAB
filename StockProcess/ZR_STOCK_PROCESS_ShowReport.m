function ZR_STOCK_PROCESS_ShowReport()
%%%%%%%% 显示报告过程
global g_tables;
global g_method;
% 开仓记录
g_tables.tabledata.record.pos=ZR_STOCK_FUN_GetTableByItems('pos');
% 交易记录       
% g_tables.tabledata.record.trade=ZR_STOCK_FUN_GetTableByItems('trade');
% 收益率表
g_tables.tabledata.returnrate=ZR_STOCK_FUN_GetReturnrateTable();
% 测评指标       
g_tables.tabledata.reference=ZR_STOCK_FUN_GetTableByItems('reference');
% 排序报告
g_tables.tabledata.sort=ZR_STOCK_FUN_GetTableByItems('sort'); 
%price
g_tables.tabledata.orderlist=ZR_STOCK_FUN_GetTableByItems('orderlist');
% 报告的输出类型
switch g_tables.outfiletype
    case 'xls'
        ...
        for l_strategyid=1:length(g_method.runstrategy)
            l_filename=strcat(l_filename,strrep(char(g_method.runstrategy(l_strategyid).fun),'ZR_STOCK_STRATEGY_','_'));
        end
...
%             xlswrite(l_filename,g_tables.tabledata.record.trade,g_tables.xls.record.trade.sheetname, 'A1');
    ...
    case 'database'  
        ZR_STOCK_PROCESS_SaveReportsetToDB();
        ZR_STOCK_PROCESS_SaveReferenceToDB();
        ZR_STOCK_PROCESS_SaveReturnrateToDB();  
        ZR_STOCK_PROCESS_SaveDailyInfoToDB();
        ZR_STOCK_PROCESS_SaveOrderlistToDB();
    case 'xml'
        ...
        if g_tables.xml.returnrate.issaved>0
            ZR_STOCK_PROCESS_SaveReturnrateToXML();
        end
        if g_tables.xml.reference.issaved>0
            ZR_STOCK_PROCESS_SaveReferenceToXML();
        end
        if g_tables.xml.dailyinfo.issaved>0
            ZR_STOCK_PROCESS_SaveDailyInfoToXML();
        end
end
