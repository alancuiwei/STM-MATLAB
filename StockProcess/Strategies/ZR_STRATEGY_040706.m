...
%     l_tradeday(l_tradeday==-1)=[];
    ...
    %==========================================================================
    %更新record中的opdate,opdateprice以及direction
    for l_tradeid=1:numel(l_oprealtradeday)
        if(l_signprice(l_oprealtradeday(l_tradeid))~=0) %判断此交点位置是否刚好为非整数
            if(l_price(1,l_oprealtradeday(l_tradeid)+1)>l_price(1,l_oprealtradeday(l_tradeid)) && l_price(1,l_oprealtradeday(l_tradeid)+1)>l_price(2,l_oprealtradeday(l_tradeid)+1)) %向上突破的条件判断
...
%                 outputdata.record.opdateprice(l_tradeid)=inputdata.commodity.serialmkdata.op(l_oprealtradeday(l_tradeid)+2)+inputdata.commodity.serialmkdata.gap(l_oprealtradeday(l_tradeid)+2);
%                 outputdata.record.direction(l_tradeid)=1;
            else if(l_price(1,l_oprealtradeday(l_tradeid))>l_price(1,l_oprealtradeday(l_tradeid)+1) && l_price(1,l_oprealtradeday(l_tradeid)+1)<l_price(2,l_oprealtradeday(l_tradeid)+1)) %向下突破的条件判断
                    if(l_oprealtradeday(l_tradeid)+2>numel(inputdata.commodity.serialmkdata.date)) %假如交点为今天和昨天之间，则更行outputdata.orderlist向量
                        outputdata.orderlist.direction=-1;
                        outputdata.orderlist.price=0;
                        outputdata.orderlist.name=inputdata.commodity.serialmkdata.ctname(l_oprealtradeday(l_tradeid)+1);
                    else 
...
%                     outputdata.record.direction(l_tradeid)=-1;
            ...
            if(l_price(1,l_oprealtradeday(l_tradeid)+1)>l_price(1,l_oprealtradeday(l_tradeid)-1) && l_price(1,l_oprealtradeday(l_tradeid)+1)>l_price(2,l_oprealtradeday(l_tradeid)+1)) %向上突破的条件判断
                ...
                outputdata.record.direction(l_tradeid)=1;
            else if(l_price(1,l_oprealtradeday(l_tradeid)+1)<l_price(1,l_oprealtradeday(l_tradeid)-1) && l_price(1,l_oprealtradeday(l_tradeid)+1)<l_price(2,l_oprealtradeday(l_tradeid)+1)) %向下突破的条件判断
                    outputdata.record.opdate(l_tradeid)=inputdata.commodity.serialmkdata.date(l_oprealtradeday(l_tradeid)+1);
                    outputdata.record.opdateprice(l_tradeid)=inputdata.commodity.serialmkdata.op(l_oprealtradeday(l_tradeid)+1)+inputdata.commodity.serialmkdata.gap(l_oprealtradeday(l_tradeid)+1);
                    outputdata.record.direction(l_tradeid)=-1;
                end
    ...
    if(l_cprealtradeday(end)+2>numel(inputdata.commodity.serialmkdata.date)) %假如交点为今天和昨天之间，则更新outputdata.orderlist向量
     ...
     for l_tradeid=1:numel(l_cprealtradeday)
        if(l_signprice(l_cprealtradeday(l_tradeid))~=0) %判断此交点位置是否刚好为非整数
            if(l_price(1,l_cprealtradeday(l_tradeid)+1)>l_price(1,l_cprealtradeday(l_tradeid)) && l_price(1,l_cprealtradeday(l_tradeid)+1)>l_price(2,l_cprealtradeday(l_tradeid)+1)) %向上突破的条件判断
                l_tempcpdate(l_tradeid)=inputdata.commodity.serialmkdata.date(l_cprealtradeday(l_tradeid)+2);
            elseif(l_price(1,l_cprealtradeday(l_tradeid))>l_price(1,l_cprealtradeday(l_tradeid)+1) && l_price(1,l_cprealtradeday(l_tradeid)+1)<l_price(2,l_cprealtradeday(l_tradeid)+1)) %向下突破的条件判断
                l_tempcpdate(l_tradeid)=inputdata.commodity.serialmkdata.date(l_cprealtradeday(l_tradeid)+2);
            end
        else %当交点位置刚好为整数时
            if(l_price(1,l_cprealtradeday(l_tradeid)+1)>l_price(1,l_cprealtradeday(l_tradeid)-1) && l_price(1,l_cprealtradeday(l_tradeid)+1)>l_price(2,l_cprealtradeday(l_tradeid)+1)) %向上突破的条件判断
                l_tempcpdate(l_tradeid)=inputdata.commodity.serialmkdata.date(l_cprealtradeday(l_tradeid)+1);
            elseif(l_price(1,l_cprealtradeday(l_tradeid)+1)<l_price(1,l_cprealtradeday(l_tradeid)-1) && l_price(1,l_cprealtradeday(l_tradeid)+1)<l_price(2,l_cprealtradeday(l_tradeid)+1)) %向下突破的条件判断
                l_tempcpdate(l_tradeid)=inputdata.commodity.serialmkdata.date(l_cprealtradeday(l_tradeid)+1);
            end
        end   
    end
