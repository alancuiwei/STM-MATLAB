...
g_method.recordvalue=@ZR_PROCESS_RecordGridSearch;
% 参数 分析参数个数，编写参数形式
l_parmstr=[];
l_disstr=[];
if nargin>0
...
l_senstr=strcat(' g_report=g_reportformat;g_method.runstrategy(',l_parmstr,'); g_method.recordvalue(',l_parmstr,'); ');
l_disstr=strcat(' disp(strcat(',l_disstr,')); ');
l_commdandstr=strcat('tic;',l_senstr,l_disstr);
